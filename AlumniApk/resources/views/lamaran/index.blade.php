            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">{{ optional($l->lowongan->perusahaan)->nama }}</td>
                        <td class="text-center">
                        @php
                            $map = [
                            'submitted'=>'primary','review'=>'info','shortlist'=>'primary',
                            'interview'=>'warning','accepted'=>'success','rejected'=>'danger','withdrawn'=>'dark'
                            ];
                        @endphp
                        <span class="badge text-{{ $map[$l->status] ?? 'secondary' }}">{{ ucfirst($l->status) }}</span>
                        </td>
                        <td class="text-center">{{ $l->created_at->format('d M Y') }}</td>
                        <td class="text-center">
